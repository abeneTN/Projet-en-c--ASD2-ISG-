...
int main(void) {
    ...
    DATE D1 ,D2 ;
    int choix;
    LISTE_CONT repertoire = CONTlisteCreer();
    ...
    do {
        ...
        case 4: 
            ...
            if (CONTlisteTaille(repertoire) == 0) {
                ...
                CONTlisteAfficher(jamaisContactes(bote_de_messagerie,repertoire));
            }
            break;
        case 5:
...
ELEMENT_MSG messageLePlusLong(LISTE_MSG liste_message) {
    ELEMENT_MSG e = MSGelementCreer(), pluslong = MSGelementCreer();
    int j; long n;
    n = 0;
    if (MSGlisteTaille(liste_message) == 0) {
        return e;
...
Pile bigrammes(LISTE_MSG L, int M) {
    ...
    do {
        message = smsTogether(p->info->msg);
        strcat(message," \0");
        printf("\n message complet : -%s- -%ld-",message,strlen(message)-1);
        k_stop = strlen(message)-2;
        printf("\n\n");
